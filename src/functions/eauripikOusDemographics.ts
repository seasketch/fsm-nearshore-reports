...
export async function eauripikOusDemographics(
  sketch:
  ...
  // Load OUS shapes
  const url = `${projectClient.dataBucketUrl()}eauripikOusDemographics.fgb`;
  const rawShapes = (await getFeaturesForSketchBBoxes(
    clippedSketch,
    url,
  )) as OusFeature[];
  console.log(`Loaded ${rawShapes.length} OUS shapes`);
  ...
  const countStats = shapes.features.reduce<OusStats>(
    (statsSoFar: OusStats, shape: OusFeature) => {
      ...
      // resp_id and island are consistent across a respondent's shapes
      ...
      const island: string = shape.properties.island
      ...
      if (!respondentProcessed[resp_id]) {
        ...
        newStats.byIsland[island] = newStats.byIsland[island]
          ? newStats.byIsland[island] + curPpl
          : curPpl;
      ...
      if (
        ...
        newStats.byIsland[island] += addnPeople;
        pplPerRespondent[resp_id] = curPpl;
        respondentProcessed[resp_id][curPpl] = true;
      }
...
export default new GeoprocessingHandler(eauripikOusDemographics, {
  title: "eauripikOusDemographics",
