...
export const Seagrass: React.FunctionComponent<ReportProps> = (props) => {
  ...
  const mg = project.getMetricGroup("seagrass", t);
    ...
    <div style={{ breakInside: "avoid" }}>
      ...
      >
        {(data: ReportResult) => {
          return (
            <>
              <Card title={t("Seagrass Habitat")}>
                <p>
                  <Trans i18nKey="Seagrass Card 1">
                    This report summarizes the percentage of seagrass habitat
                    area within
                  </Trans>{" "}
                  {curGeography.display}
                  <Trans i18nKey="Seagrass Card 2">
                    's state waters that overlap with this plan. Plans should
                    consider protection of seagrass habitat for conservation.
                  </Trans>
                </p>
